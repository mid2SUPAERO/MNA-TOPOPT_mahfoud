%% ***************** truss model code with MNA method ****************** %%
clear ; close all ;
%% DATA
load V ;
nelx = 80 ;
...
volfrac = 0.4 ;
lc = sqrt(volfrac*nelx*nely/(0.5*nX*nY));
...
x = [reshape(xElts,1,numel(xElts));
    ...
    zeros(1,numel(xElts));
    lc*ones(2,numel(xElts))];
x = reshape(x,numel(x),1);
Xmin    = repmat([0;0;-pi;0;0],length(x)/5,1);
Xmax    = repmat([nelx;nely;pi;nelx/4;nely/4],length(x)/5,1);
x = (x-Xmin)./(Xmax-Xmin) ;
...
while kktnorm > kkttol && outit < maxoutit
    ...
    for in=1:length(X)/5  % loop on the components
        ...
        neigh = [ ((xN-X(q))*cn + (yN-X(q+1))*sn)/X(q+3);
            (-(xN-X(q))*sn + (yN-X(q+1))*cn)/X(q+4)];
        ...
        % COMPUTE DENSITIES
        if(~isempty(ind))
            % shape function andderivatives calculation
            v = neigh(:,nc) ;
            sx = sign(v(1,:)');
            sy = sign(v(2,:)');
            v = abs(v);
            ...
            wx = zeros(size(v,2),1);
            wy = zeros(size(v,2),1);
            dWdx = zeros(size(v,2),1);
            dWdy = zeros(size(v,2),1);
            if outit < 200
                wx(p1) = 1-6*v(1,p1).^8+6*v(1,p1).^12;
                wx(~p1) = 2-6*v(1,~p1)+6*v(1,~p1).^2-2*v(1,~p1).^3;
                wy(p2) = 1-6*v(2,p2).^8+6*v(2,p2).^12;
                wy(~p2) = 2-6*v(2,~p2)+6*v(2,~p2).^2-2*v(2,~p2).^3;
            else
                wx(p1) = 1-6*v(1,p1).^8+6*v(1,p1).^12;
                wx(~p1) = V(1)+V(2)*v(1,~p1).^2+V(3)*v(1,~p1).^4+V(4)*v(1,~p1).^6+V(5)*v(1,~p1).^8+V(6)*v(1,~p1).^12 ;
                wy(p2) = 1-6*v(2,p2).^8+6*v(2,p2).^12;
                wy(~p2) = V(1)+V(2)*v(1,~p2).^2+V(3)*v(1,~p2).^4+V(4)*v(1,~p2).^6+V(5)*v(1,~p2).^8+V(6)*v(1,~p2).^12 ;
            end
            W = abs(wx.*wy) ;
            if outit < 200
                dWdx(p1) = (-12*v(1,p1)+18*v(1,p1).^2) ;
                dWdx(~p1) = (-6+12*v(1,~p1)-6*v(1,~p1).^2) ;
                dWdx = dWdx.*sx.*wy ;
                dWdy(p2) = (-12*v(2,p2)+18*v(2,p2).^2) ;
                dWdy(~p2) = (-6+12*v(2,~p2)-6*v(2,~p2).^2) ;
                dWdy = dWdy.*sy.*wx;
            else
                dWdx(p1) = (-48*v(1,p1).^7+72*v(1,p1).^11) ;
                dWdx(~p1) = 2*V(2)*v(1,~p1)+4*V(3)*v(1,~p1).^3+6*V(4)*v(1,~p1).^5+8*V(5)*v(1,~p1).^7+12*V(6)*v(1,~p1).^11 ;
                dWdx = dWdx.*sx.*wy ;
                dWdy(p2) = (-48*v(2,p2).^7+72*v(2,p2).^11) ;
                dWdy(~p2) = 2*V(2)*v(1,~p2)+4*V(3)*v(1,~p2).^3+6*V(4)*v(1,~p2).^5+8*V(5)*v(1,~p2).^7+12*V(6)*v(1,~p2).^11 ;
                dWdy = dWdy.*sy.*wx ;
            end
            dWdx = dWdx/X(q+3); dWdy = dWdy/X(q+4);
            d(ind) = d(ind) + W;
            dd(ind,q) = - cn*dWdx + sn*dWdy;
            dd(ind,q+1) = - sn*dWdx - cn*dWdy;
            dd(ind,q+2) = (-(xN(nc)-X(q))*sn + (yN(nc)-X(q+1))*cn)'.*dWdx - ...
                ((xN(nc)-X(q))*cn + (yN(nc)-X(q+1))*sn)'.*dWdy;
            dd(ind,q+3) = - ((xN(nc)-X(q))*cn + (yN(nc)-X(q+1))*sn)'.*dWdx/X(q+3);
            dd(ind,q+4) = ((xN(nc)-X(q))*sn - (yN(nc)-X(q+1))*cn)'.*dWdy/X(q+4);
        end
    end
    % ASYMPTOTIC DENSITY
    ...
    for i=1:length(X)/5
        ...
        sn = sin(X(q+2));
        cn = cos(X(q+2));
        fill([X(q)+fact*X(q+3)*cn-fact*X(q+4)*sn,...
            X(q)-fact*X(q+3)*cn-fact*X(q+4)*sn,...
            X(q)-fact*X(q+3)*cn+fact*X(q+4)*sn,...
            X(q)+fact*X(q+3)*cn+fact*X(q+4)*sn,...
            X(q)+fact*X(q+3)*cn-fact*X(q+4)*sn],...
            [X(q+1)+fact*X(q+3)*sn+fact*X(q+4)*cn,...
            X(q+1)-fact*X(q+3)*sn+fact*X(q+4)*cn,...
            X(q+1)-fact*X(q+3)*sn-fact*X(q+4)*cn,...
            X(q+1)+fact*X(q+3)*sn-fact*X(q+4)*cn,...
            X(q+1)+fact*X(q+3)*sn+fact*X(q+4)*cn],[0 0.2 0.4])
    end
    hold off

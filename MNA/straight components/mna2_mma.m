...
ratio=1.2;         % tolerence on the node centroide and Lx,Ly
tolchange=1e-2;
...
d = sqrt(volfrac*nelx*nely/(4*nX*nY));
...
Xmin    = repmat([0;0;0;0;0],length(x)/5,1);
Xmax    = repmat([nelx;nely;nelx;nely;nely],length(x)/5,1);
x = (x-Xmin)./(Xmax-Xmin) ;
%% saving pictures
...
while (kktnorm > kkttol && outit < maxoutit)&&change>tolchange
    ...
    for in=1:length(X)/5  % loop on the components
        ...
        if(~isempty(ind))
            ...
            dfdx(p1) = (-48*v(1,p1).^7+72*v(1,p1).^11) ;
            dfdx(~p1) = 2*V(2)*v(1,~p1)+4*V(3)*v(1,~p1).^3+6*V(4)*v(1,~p1).^5+8*V(5)*v(1,~p1).^7+12*V(6)*v(1,~p1).^11 ;
            dfdx = dfdx.*sx.*wy ;
            dfdy(p2) = (-48*v(2,p2).^7+72*v(2,p2).^11) ;
            dfdy(~p2) = 2*V(2)*v(1,~p2)+4*V(3)*v(1,~p2).^3+6*V(4)*v(1,~p2).^5+8*V(5)*v(1,~p2).^7+12*V(6)*v(1,~p2).^11 ;
            dfdy = dfdy.*sy.*wx ;

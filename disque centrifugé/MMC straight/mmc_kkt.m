...
volfrac = 0.3 ;
%% saving pictures
% parent_dir_name = 'mesh100x100_v0.5_16x16\';
...
itor = 0.2/length(angle)*[cos(angle);sin(angle)] ; itor = itor(:) ;
...
%     dn = zeros((nelx+1)*(nely+1),1) ; ddn = zeros((nelx+1)*(nely+1),nn) ;
%     for e = 1:nelx*nely
%         dn(EleNodesID(e,:)) = dn(EleNodesID(e,:)) + den(e)/4 ;
%         ddn(EleNodesID(e,:),:) = ddn(EleNodesID(e,:),:) + dden(e,:)/4 ;
%     end
%     F = reshape([dn';dn'],[],1) ;
    F = reshape([H;H],[],1) ;
    F = F.*fcv + F_tor ;
...
%     end
    for j = 1:N
        for ii = 1:Var_num
            k = ii+Var_num*(j-1) ;
            dF(:,k) = reshape([diffH{j}(:,ii)';diffH{j}(:,ii)'],[],1).*fcv ;
        end
    end
...
title('historique de convergence de la norme kkt') ; toc

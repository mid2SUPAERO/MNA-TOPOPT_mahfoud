...
x_int = 0.5 ;
y_int = 0.5 ;
ini_val = [0.4 0.04 pi/4 -1] ;
volfrac = 0.3 ;
%% saving pictures
parent_dir_name = 'mesh100x100_v0.3_16x16\';
...
teta=repmat([ini_val(3) -ini_val(3)],1,N/2); % vector of the sine value of the inclined angle of each component
cr=repmat([ini_val(4) -ini_val(4)],1,N/2);
variable=[x0;y0;L;t;teta;cr];
N = length(x0) ;
%% Parameters of MMA
xy00=variable(:);
xval=xy00;
...
xmin=[0 ; 0  ; 0.03 ; 0.01 ; -2*pi ; -2];
xmin=repmat(xmin,N,1);
xmax=[DW ; DH ; 1 ; 0.2 ; 2*pi ; 2];

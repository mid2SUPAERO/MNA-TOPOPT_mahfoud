...
nely = 10*10;          % Number of finite elements along y
volfrac = 0.5 ;     % Targeted Volume fraction
penal0 = 3 ;
% Starting configuration
nX = 10 ;             % Number of deformable elements along x
nY = 10 ;             % Number of deformable elements along y
tolchange=1e-3 ;
...
x = (x-Xmin)./(Xmax-Xmin) ;
%% saving pictures
parent_dir_name = 'mesh100x100_pur_torsion_v0.5_10x10\';
...
while (kktnorm > kkttol && outit < maxoutit)&&change>tolchange
    ...
    dn = zeros((nelx+1)*(nely+1),1) ; ddn = zeros((nelx+1)*(nely+1),n_tot) ;
    for e = 1:nelx*nely

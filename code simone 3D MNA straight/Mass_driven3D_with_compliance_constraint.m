...
tic
E0=210000;
[FEM_structure]= Stiffness_components3D_compliance(FEM_structure,E0);
toc
%% INITIALIZE ITERATION
volfrac=0.1;
admissible_DTSFC=0.15;
admissible_Compliance=4500;
penal=3; ft=2;
...
[C,dC,perfo,dperfo,GV,dGv,GKSl,dGKSl,FEM_structure]=FEA3D_compliance(FEM_structure,xPhys,E0,penal,volfrac,mgcg_selector,nfin,Engine_master,VMl,Penalty,toll_minres_U,toll_minres_TSFC,toll_minres_GKS);
...
Gc=(C-admissible_Compliance)/admissible_Compliance;
dGc=dC/admissible_Compliance;
Vol=(1+GV)*volfrac;
dVol=dGv*volfrac;
f0val=Vol*100;
...
%color filter
FACES_shown1=FACES(Fac_color<=0.7,:);
Fac_color1=Fac_color(Fac_color<=0.7);
FACES_shown2=FACES(Fac_color<=0.1,:);
...
% end
save(['convergence_',num2str(outit,'%03d')],'outeriter','perfo','GKSl','Vol','C')
...
while kktnorm > kkttol & outit < maxoutit
    ...
    Maff=1-Optistruc_affich;
    %color filter
    FACES_shown1=FACES(Fac_color<=Maff,:);

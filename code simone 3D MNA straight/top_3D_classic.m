...
sK=k_ij.*E;
tic
K_D = sparse(I,J,sK,size(K_def,1),size(K_def,2)); K_D=K_D+triu(K_D.',1);K_D=(K_D+K_D.')/2;
K=K_def+K_D;
% Kcomp=K_def_compliance+K_D;
assembly_time=toc
F=[LOAD_MATRIX_def(:,1)];
tic
...
U(free_dofs)=Res(:,1);
tic
[dK_DXs]=Kdudxcompl_eval(xPhys,el_ij,I,J,dK,K_def,U,disp_DOF);
gradient_evaluation=toc
tic
dK_dxU=zeros(length(U(disp_DOF)),length(xPhys(:)));
for xdim=1:length(xPhys(:))
    dK_dxU(:,xdim)=dK_DXs(xdim).mat(disp_DOF,disp_DOF)*U(disp_DOF);
end
dk_dxu_eval=toc
...
tic
C = U(disp_DOF).'*K(disp_DOF,disp_DOF)*U(disp_DOF);
compliance_eval=toc
tic
dC= -U(disp_DOF).'*dK_dxU;
complance_grad=toc
...
C0=1;
f0val=C/C0;
fval=[(sum(xPhys(:).*A2/2) - volfrac*sum(A2/2))/(volfrac*sum(A2/2))];
df0dx=reshape(dC,[],1)/C0;
dfdx=[dv(:)'/(volfrac*sum(A2/2))];
innerit=0;
...
while change > 0.0001
    ...
    while (l2-l1)/(l1+l2) > 1e-3
        ...
        if (sum(xPhys(:).*A2/2)>volfrac*sum(A2/2)), l1 = lmid; else l2 = lmid; end
    end

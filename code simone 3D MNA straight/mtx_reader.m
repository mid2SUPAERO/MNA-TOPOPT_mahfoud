...
while ((feof(fid) == 0)&&(flag == 0))
    ...
    if strcmp(Line(1:13),'*USER ELEMENT');
        ...
        %DOFs_map 1 3
        Retaine_DOFs_Number=2*Number_of_interface_nodes+6*sum(Interface_GID==0);
        DOFs_MAP=zeros(Retaine_DOFs_Number,4);
        for k=1:Number_of_interface_nodes
            DOFs_MAP((k-1)*2+(1:2),:)=[Interface_GID(k)*ones(2,1),Interface_local_GID(k)*ones(2,1),[1;3],(k-1)*2+[1;2]];
        end
        for k=(Number_of_interface_nodes+1):(Number_of_interface_nodes+sum(Interface_GID==0))
            DOFs_MAP(Number_of_interface_nodes*2+6*(k-Number_of_interface_nodes-1)+(1:6),:)=[Interface_GID(k)*ones(6,1),Interface_local_GID(k)*ones(6,1),[1;2;3;4;5;6],Number_of_interface_nodes*2+(k-Number_of_interface_nodes-1)*6+[1;2;3;4;5;6]];
        end
    ...
    if strcmp(Line(1:length('** SUBSTRUCTURE RECOVERY VECTOR')),'** SUBSTRUCTURE RECOVERY VECTOR')
        ...
        c=0;
        for l=1:line_number-1*(line_number~=6*Number_of_tip_nodes/4)
            ...
            c=c+el;
            if line_number~=6*Number_of_tip_nodes/4
                Line = [fgetl(fid) blanks(length('** SUBSTRUCTURE RECOVERY VECTOR'))];
                count = count + 1; LowBound = floor(count/BlockSize)*BlockSize;
                if (Bound~=LowBound)
                    Bound = LowBound;

function [FEM_structure]= Stiffness_components3Dpar(FEM_structure,E0)
%% Extract inputs
ELEMENT=[(1:size(FEM_structure.ELEMENT,1))',FEM_structure.ELEMENT];
...
DESIGN_ZONE_COORD=[(1:size(FEM_structure.COORD,1)).',FEM_structure.COORD];
...
x1=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,1),1);
y1=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,1),2);
z1=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,1),3);
x2=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,2),1);
y2=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,2),2);
z2=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,2),3);
x3=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,3),1);
y3=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,3),2);
z3=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,3),3);
x4=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,4),1);
y4=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,4),2);
z4=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,4),3);
x5=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,5),1);
y5=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,5),2);
z5=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,5),3);
x6=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,6),1);
y6=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,6),2);
z6=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,6),3);
x7=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,7),1);
y7=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,7),2);
z7=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,7),3);
x8=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,8),1);
y8=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,8),2);
z8=FEM_structure.COORD(DESIGN_ZONE_ELEMENT_NODES(:,8),3);
%stiffness matrix
...
Jn=In;
Nnn=In;
I=zeros(size(detJ,1),300*1);
J=zeros(size(detJ,1),300*1);
Ncomp=960;
Is=zeros(size(detJ,1),Ncomp);
Js=zeros(size(detJ,1),Ncomp);
DBs=zeros(size(detJ,1),Ncomp);
k_ij=J;
el_ij=k_ij;
IK0=zeros(size(detJ,1),24*24*1);
JK0=zeros(size(detJ,1),24*24*1);
KV0=IK0;
parfor l=1:size(detJ,1)
    Ke_E=zeros(24,24);
    DB=zeros(6*8,24);
    for m=1:8
...
%     Ke_E=detJ(l,1)*BTDB(:,:,1)+detJ(l,2)*BTDB(:,:,2)+detJ(l,3)*BTDB(:,:,3)+detJ(l,4)*BTDB(:,:,4)+detJ(l,5)*BTDB(:,:,5)+detJ(l,6)*BTDB(:,:,6)+detJ(l,7)*BTDB(:,:,7)+detJ(l,8)*BTDB(:,:,8);
    ...
    for i3=1:300
    I(i3,l)=reshape(DESIGN_ZONE_ELEMENT_DOFS(l,IND_I(i3)),[],1);
    J(i3,l)=reshape(DESIGN_ZONE_ELEMENT_DOFS(l,IND_J(i3)),[],1);
    k_ij(i3,l)=Kv(i3);
    el_ij(i3,l)=l;
    end
    [Ii,Ji,KV]=find((Ke_E));
...
%     IK0(64*(l-1)+(1:64)',1)=reshape(DESIGN_ZONE_ELEMENT_DOFS(l,Ii),[],1);
    IK0(i5,l)=Ii(i5)+(24)*(l-1);
    JK0(i5,l)=reshape(DESIGN_ZONE_ELEMENT_DOFS(l,Ji(i5)),[],1);
    KV0(i5,l)=KV(i5);
    end
    for inc=1:numb_comp_nn
    In(inc,l)=48*(l-1)+Indgp(inc);
    Jn(inc,l)=6*(node_id_n(l,indn(inc))-1)+node_id_s(l,indn(inc));
    Nnn(inc,l)=Nn(inc);
    end
    ...
    for nc=1:Ncomp
        if nc<=length(IND_si)
            Is(nc,l)=48*(l-1)+IND_si(nc);
            Js(nc,l)=reshape(DESIGN_ZONE_ELEMENT_DOFS(l,IND_sj(nc)),[],1);
            DBs(nc,l)=E0*dbs(nc);
        end
    end
%     disp(num2str(l/size(detJ,1)*100))
end
IK0=IK0(:);JK0=JK0(:);KV0=KV0(:);
K0c=sparse((IK0),(JK0),(KV0),size(detJ,1)*24,max(max(DESIGN_ZONE_ELEMENT_DOFS)));
clear IK0 JK0 KV0
I=I(:);J=J(:);k_ij=k_ij(:);el_ij=el_ij(:);
In=In(:);Jn=Jn(:);Nnn=Nnn(:);
Is=Is(:);Js=Js(:);DBs=DBs(:);
Is=setdiff(Is,0,'stable');
Js=setdiff(Js,0,'stable');
DBs=setdiff(DBs,0,'stable');
clear BTDB DBe
i1=I;
j1=J;
i1(I<J)=J(I<J);
...
Lagrangian_multip=FEM_structure.new_DOFs_MAP(end-sum(FEM_structure.new_DOFs_MAP(:,1)==0)+1:end,4);
...
psi_tild=zeros(ndof,2); Sol=zeros(length(FEM_structure.observation_dofs)+size(FEM_structure.K_interface,1),1); psi_tild_sol=[Sol,Sol];

function [FEM_structure]= Stiffness_components3D(FEM_structure,E0)
%% Extract inputs
...
psi_tild=zeros(ndof,2); Sol=zeros(length(FEM_structure.observation_dofs)+size(FEM_structure.K_interface,1),1); 
psi_tild_sol=zeros(length(FEM_structure.observation_dofs)+size(FEM_structure.K_interface,1),2);

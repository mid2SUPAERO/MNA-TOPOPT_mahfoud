function [perfo,dperfo,GV,dGv,GKSl,dGKSl,FEM_structure]=FEA3Dpar(FEM_structure,xPhys,E0,penal,volfrac,mgcg_selector,nl,Engine_master,VMl,Penalty,toll_minres_U,toll_minres_TSFC,toll_minres_GKs)
% inputs
...
if mgcg_selector
    ...
    if ~Engine_master
        ...
        K=distributed(K_def+K);
    else
        K = (sparse(FEM_structure.I,FEM_structure.J,sK,size(K_def,1),size(K_def,2))); K=K+triu(K.',1); K=(K+K.')/2;
        K=distributed(FEM_structure.K_Engine+FEM_structure.Pr_EM'*K*FEM_structure.Pr_EM);
    end
end
if ~Engine_master
    F=distributed(FEM_structure.LOAD_MATRIX_def(:,1));
else
    F=distributed(FEM_structure.LOAD_MATRIX_EM(:,1));
end
...
if mgcg_selector
    ...
    if length(free_dofs)<1e10||toll_minres_U==0
        if ~Engine_master
            tic
            spmd; Res=K(free_dofs,free_dofs)\F(free_dofs,:);U(free_dofs)=Res(:,1); end
            toc
            U=gather(U);
        else
            tic
           spmd; Res=K(FEM_structure.free_dofs_engine_master,FEM_structure.free_dofs_engine_master)\F(FEM_structure.free_dofs_engine_master,:); end 
           Sol(FEM_structure.free_dofs_engine_master)=(Res);
            U=gather(FEM_structure.Pr_EM*Sol);
            toc
            Sol=gather(Sol);
        end
    else
...
%                     alpha=0.1;
            L1 = ichol(K(free_dofs,free_dofs), struct('type','ict','droptol',1e-3,'diagcomp',alpha/100));
            maxit=1000;
            ...
            alpha = (max(sum(abs(K(FEM_structure.free_dofs_engine_master,FEM_structure.free_dofs_engine_master)),2)./diag(K(FEM_structure.free_dofs_engine_master,FEM_structure.free_dofs_engine_master)))-2)/10;
%                     alpha=0.1;
            tic
            r = symrcm(K(FEM_structure.free_dofs_engine_master,FEM_structure.free_dofs_engine_master));
            L1 = ichol(K(FEM_structure.free_dofs_engine_master(r),FEM_structure.free_dofs_engine_master(r)), struct('type','ict','droptol',1e-3,'diagcomp',alpha/100));
...
% toc
xPhys=xPhys(:);
CdK=(penal*(E0-FEM_structure.Emin)*(xPhys(:)).^(penal-1));
...
Ngp=length(RO);
[gmax]=max(gj);
GKSl=gather((gmax-log(Ngp)/Penalty+1/Penalty*log(sum(exp(Penalty*(gj-gmax))))));
...
if ~mgcg_selector
    if length(free_dofs)<1e10||toll_minres_U==0
        if ~Engine_master
            tic
           spmd; RES2=K(free_dofs,free_dofs)\[DTSFC_DU(free_dofs),Tau(free_dofs)'];end
           psi_tild(free_dofs,:)=gather(RES2);
           toc
%             RES2=K(free_dofs,free_dofs)\[DTSFC_DU(free_dofs)];
        else
            tic
            spmd; RES2=K(FEM_structure.free_dofs_engine_master,FEM_structure.free_dofs_engine_master)\[DTSFC_DU(FEM_structure.free_dofs_engine_master,:),Tau(FEM_structure.free_dofs_engine_master)'];end
            toc
            psi_tild_sol(FEM_structure.free_dofs_engine_master,:)=gather(RES2);
            psi_tild=gather(FEM_structure.Pr_EM*psi_tild_sol);
        end
    else
        %          alpha=0.1;

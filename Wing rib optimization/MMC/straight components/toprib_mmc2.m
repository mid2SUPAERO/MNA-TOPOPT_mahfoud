...
%% initial design
nX = 20 ; % number of components along x axis
nY = 5 ; % number of components along y axis
l_ini = sqrt(volfrac*DW*DH/(0.5*nX*nY)) ; % initial length of component
% [xElts,yElts] = meshgrid(linspace(1/(nX+1)*DW,nX/(nX+1)*DW,nX),...
%     linspace(1/(nY+1)*DH,nY/(nY+1)*DH,nY)) ;
...
% n_c = length(xElts) ;
% % x = [xElts;yElts;zeros(1,n_c);4*l_ini*ones(1,n_c);1/4*l_ini*ones(1,n_c)] ;
% x = [xElts;yElts;pi/4*ones(1,n_c);4*l_ini*ones(1,n_c);1/4*l_ini*ones(1,n_c)] ;
% n_var = numel(x) ;
% x = reshape(x,n_var,1) ;
...
n_c = length(xElts) ;
x = [xElts;yElts;repmat([pi/8 -pi/8],1,n_c/2);3*l_ini*ones(1,n_c);1/3*l_ini*ones(1,n_c)] ;
n_var = numel(x) ;
x = reshape(x,n_var,1) ;

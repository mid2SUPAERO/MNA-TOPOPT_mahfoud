...
% §§§§§ try penalization law instead of ersatz material §§§§
clear ; close all ;
%% data
nelx = 160 ;
nely = 40 ;
...
n_c = length(xElts) ;
x = [xElts;yElts;repmat([pi/4 -pi/4],1,n_c/2);3*l_ini*ones(1,n_c);1/3*l_ini*ones(1,n_c)] ;
n_var = numel(x) ;
x = reshape(x,n_var,1) ;
...
% % reshaping
% F1 = F(:,1) ; F2 = F(:,2) ;
% F11 = F1(1:2:end) ; F12 = F1(2:2:end) ;
% F21 = F2(1:2:end) ; F22 = F2(2:2:end) ;
% F11 = reshape(F11,nely+1,nelx+1) ; F11 = flipud(F11) ; F11 = F11(:) ;
% F12 = reshape(F12,nely+1,nelx+1) ; F12 = flipud(F12) ; F12 = F12(:) ;
% F21 = reshape(F21,nely+1,nelx+1) ; F21 = flipud(F21) ; F21 = F21(:) ;
% F22 = reshape(F22,nely+1,nelx+1) ; F22 = flipud(F22) ; F22 = F22(:) ;
...
Xmax = repmat([DW;DH;pi;DW;DH],n_c,1) ;
x = (x-Xmin)./(Xmax-Xmin) ;
...
while change > 0.1*tol && loop <= maxoutit && kktnorm > tol
    if loop > 15
        alpha = 1e-3 ;
    end
    loop = loop+1 ;
...
%     delta = 0.1 ;
    ...
    for j = 1:n_c
        for i = 1:5
            dH_dxi = diffH{j}(:,i) ;
            grad_den(:,5*(j-1)+i) = 1/4*sum(dH_dxi(EleNodesID),2) ;
        end
    end
    A1=sum(den)*EW*EH ;
